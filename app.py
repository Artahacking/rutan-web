...
def template_litmas():
    # Membuat Template sesuai format User (Header di baris 3)
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        # Trik untuk membuat Judul di atas
        ...
        # Header kolom di baris ke-3 (Excel Row 3, Pandas Index 2)
...
# --- ADMIN KELOLA LITMAS (DENGAN UPLOAD EXCEL) ---
...
def admin_litmas_add():
    ...
    if file.filename == '':
        flash('Belum memilih file', 'danger')
        return redirect(url_for('admin_litmas'))

    if file and (file.filename.endswith('.xlsx') or file.filename.endswith('.xls')):
        try:
            # PENTING: header=2 artinya membaca header dari Baris ke-3 Excel
            # Karena di file Anda baris 1 adalah Judul, baris 2 kosong/judul lanjutan
            ...
            for index, row in df.iterrows():
                # Menggunakan indeks kolom (iloc) agar lebih aman
                # Kolom 0 = No (skip)
                # Kolom 1 = Nama
                # Kolom 2 = Pidana Thn
                # Kolom 3 = Pidana Bln
                ...
                # Cek jika baris kosong (Nama NaN)
            ...
            conn.commit()
            conn.close()
            flash(f'Berhasil mengimpor {count_success} data Litmas dari Excel!', 'success')
...
# --- ADMIN KELOLA WBP BEBAS (DENGAN UPLOAD EXCEL) ---
...
def admin_bebas_add():
    ...
    if file and (file.filename.endswith('.xlsx') or file.filename.endswith('.xls')):
        try:
            # PENTING: header=2 (Membaca dari baris ke-3)
            df = pd.read_excel(file, header=2)
            conn = get_db()
            cursor = conn.cursor()
            ...
            for index, row in df.iterrows():
                ...
                # Parsing Tanggal (Format di Excel Anda d/m/yyyy)
                tgl_str = datetime.now().strftime('%Y-%m-%d')
                if pd.notna(raw_tgl):
                    try:
                        # Coba parse sebagai datetime pandas
                        tgl_date = pd.to_datetime(raw_tgl, dayfirst=True) # dayfirst=True penting untuk format Indo (DD/MM/YYYY)
                        tgl_str = tgl_date.strftime('%Y-%m-%d')
                    except:
                        # Jika gagal (misal teks aneh), pakai tanggal hari ini
                        pass
            ...
            conn.commit()
            conn.close()
            flash(f'Berhasil mengimpor {count_success} data WBP Bebas dari Excel!', 'success')
            
        except Exception as e:
            flash(f'Terjadi kesalahan (Format Excel salah?): {str(e)}', 'danger')
    else:
        flash('Format file harus Excel (.xlsx / .xls)', 'danger')
